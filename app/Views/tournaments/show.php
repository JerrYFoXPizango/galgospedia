    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4" style="position:relative;z-index:2">
            ...
            <div id="poster-lightbox"
                ...
                <button class="absolute top-4 right-4 text-white text-3xl leading-none font-bold opacity-70 hover:opacity-100"
                        onclick="document.getElementById('poster-lightbox').classList.add('hidden')">&times;</button>
                <img src="<?= $posterUrl ?>"
                     alt="Cartel de <?= htmlspecialchars($tournament['title']) ?>"
            ...
            document.addEventListener('keydown', function(e){
                if(e.key === 'Escape') document.getElementById('poster-lightbox').classList.add('hidden');
            });
            </script>
            <?php endif; ?>
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-5">
                <h2 class="font-semibold text-amber-800 flex items-center gap-2 mb-2">
                    ...
                    <?php if ($tournament['meeting_time']): ?>
                        <span class="ml-auto text-sm font-normal bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
                            🕐 <?= substr($tournament['meeting_time'], 0, 5) ?>h
                        </span>
                    <?php endif; ?>
                </h2>
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                <dl class="space-y-2 text-sm">
                    ...
                    <div class="flex gap-2">
                        <dt class="text-gray-500 w-28 flex-shrink-0">Organizador</dt>
                        <dd class="text-gray-800 font-medium"><?= htmlspecialchars($tournament['organizer_name']) ?></dd>
                    </div>
                    <?php endif; ?>
                    <?php if ($tournament['contact_info']): ?>
                    <div class="flex gap-2">
                        <dt class="text-gray-500 w-28 flex-shrink-0">Contacto</dt>
                        <dd class="text-gray-800"><?= htmlspecialchars($tournament['contact_info']) ?></dd>
                    </div>
                    <?php endif; ?>
                    <?php if ($tournament['max_participants']): ?>
                    <div class="flex gap-2">
                        <dt class="text-gray-500 w-28 flex-shrink-0">Plazas</dt>
                        <dd class="text-gray-800"><?= $tournament['max_participants'] ?></dd>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="flex gap-2 items-start">
                        ...
                        <dd>
                            ...
                            <?php if ($tournament['registration_url']): ?>
                                <a href="<?= htmlspecialchars($tournament['registration_url']) ?>" target="_blank" rel="noopener"
                                   class="ml-2 text-xs text-galgo-red hover:underline">→ Inscribirse</a>
                            <?php endif; ?>
                        </dd>
                    </div>
                    <?php endif; ?>
        ...
        <div class="space-y-4" style="position:relative;z-index:1">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="p-4 space-y-2">
                    <p class="text-xs text-gray-500 font-medium uppercase tracking-wide mb-2">Navegar hasta aquí</p>
                    <a href="https://www.google.com/maps?q=<?= $lat ?>,<?= $lng ?>" target="_blank" rel="noopener"
                       class="flex items-center gap-2 w-full px-3 py-2 rounded-lg border border-gray-200 text-sm hover:border-blue-400 hover:bg-blue-50 transition-colors text-gray-700 font-medium">
                    ...
                    <a href="https://waze.com/ul?ll=<?= $lat ?>,<?= $lng ?>&amp;navigate=yes" target="_blank" rel="noopener"
                       class="flex items-center gap-2 w-full px-3 py-2 rounded-lg border border-gray-200 text-sm hover:border-sky-400 hover:bg-sky-50 transition-colors text-gray-700 font-medium">
                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="#33CCFF" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 2C6.48 2 2 6.48 2 12c0 3.93 2.27 7.34 5.57 9.07L8 22l.5-1.94C9.58 20.64 10.77 21 12 21c5.52 0 10-4.48 10-10S17.52 2 12 2z"/>
                        </svg>
                        Waze
                    </a>
                    <a href="https://maps.apple.com/?daddr=<?= $lat ?>,<?= $lng ?>&amp;dirflg=d" target="_blank" rel="noopener"
                       class="flex items-center gap-2 w-full px-3 py-2 rounded-lg border border-gray-200 text-sm hover:border-gray-400 hover:bg-gray-50 transition-colors text-gray-700 font-medium">
                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="#000" xmlns="http://www.w3.org/2000/svg">
            ...
            <!-- Compartir -->
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide mb-2">Compartir</p>
                <button onclick="copyLink()" id="share-btn"
                        class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm hover:border-gray-400 transition text-gray-600">
                    📋 Copiar enlace
                </button>
                <script>
                function copyLink() {
            ...
            <!-- Publicado por -->
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 text-xs text-gray-400">
                Publicado por <span class="font-medium text-gray-600"><?= htmlspecialchars($tournament['creator_username'] ?? '—') ?></span><br>
            ...
            <?php if ($canEdit): ?>
            <div class="flex gap-2">
                <a href="/torneos/<?= $tournament['slug'] ?>/editar" class="btn-outline text-sm flex-1 text-center">Editar</a>
                <form method="POST" action="/torneos/<?= $tournament['slug'] ?>/eliminar"
